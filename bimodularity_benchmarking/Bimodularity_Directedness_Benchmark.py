...
def adjacency_matrix_directedness_transform(adj_matrix, gamma):
    ...
    result = np.divide(A, div, out= np.zeros_like(A), where=(A + A.T) != 0)  # Avoid division by zero
...
def community_fit_w_tolerance(sending_communities, receiving_communities,  epsilon = 0.5):
    ...
    -----------
    sending_communities : numpy.ndarray
        Array of sending community inxey for each node
    receiving_communities : numpy.ndarray
        Array of receiving community index for each node
...
def get_surviving_communities(communities, tolerance, k,  alphas, gammas):



    counts = np.array([[0] *(k+1)] * k) 
    for i, g_com in enumerate(communities): 
        for j, (sending_communities, receiving_communities) in enumerate(g_com):
            fit_result = community_fit_w_tolerance(sending_communities, receiving_communities, tolerance)
            
            # Extract surviving indices
            singular_surviving_indices = fit_result[0][1]  # singular community match indices
            c = len(singular_surviving_indices)
            counts[c] = counts[c] +  [1 if i in singular_surviving_indices else 0 for i in range (k)]
