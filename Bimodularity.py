...
def modify_adjacency_matrix(adj_matrix, gamma):
    """
    ...
    A = np.array(A)
    result = 2* A * A**gamma / (A**gamma + A.T**gamma)
    result[np.isnan(result)] = 0  # Handle NaN values if any division by zero occurs
    return result
...
def get_concat_binarized_bycommunities(sending_communities, receiving_communities):
    ...
    return (bicommunities >= 0.5).astype(int)
